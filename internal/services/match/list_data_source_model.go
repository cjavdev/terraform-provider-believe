...
type MatchesDataListDataSourceEnvelope struct {
	Data customfield.NestedObjectList[MatchesItemsDataSourceModel] `json:"data,computed"`
}

type MatchesDataSourceModel struct {
	...
	Items     customfield.NestedObjectList[MatchesItemsDataSourceModel] `tfsdk:"items"`
}

func (m *MatchesDataSourceModel) toListParams(_ context.Context) (params believe.MatchListParams, diags diag.Diagnostics) {
	params = believe.MatchListParams{}

	if !m.MatchType.IsNull() {
		params.MatchType = believe.MatchType(m.MatchType.ValueString())
	}
	if !m.Result.IsNull() {
		params.Result = believe.MatchResult(m.Result.ValueString())
	}
	if !m.TeamID.IsNull() {
...
type MatchesItemsDataSourceModel struct {
	...
	TurningPoints        customfield.NestedObjectList[MatchesTurningPointsDataSourceModel] `tfsdk:"turning_points" json:"turning_points,computed"`
	WeatherTempCelsius   types.Float64                                                     `tfsdk:"weather_temp_celsius" json:"weather_temp_celsius,computed"`
}

type MatchesTurningPointsDataSourceModel struct {
