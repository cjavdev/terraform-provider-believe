...
var _ datasource.DataSourceWithConfigure = (*QuoteDataSource)(nil)

func NewQuoteDataSource() datasource.DataSource {
...
func (d *QuoteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if data.FindOneBy != nil {
		...
		resp.Diagnostics.Append(diags...)
		data.QuoteID = ts[0].ID
	}

	res := new(http.Response)
	_, err := d.client.Quotes.Get(
		ctx,
		data.QuoteID.ValueString(),
